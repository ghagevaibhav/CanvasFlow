...
const DemoCanvas = () => {
  ...
  const [tool, setTool] = useState<'brush' | 'rectangle' | 'circle' | 'eraser' | 'move' | 'line'>('brush');
  ...
  const [action, setAction] = useState<'none' | 'moving' | 'rotating' | 'resizing'>('none');
  ...
  useEffect(() => {
    ...
    if (savedState && savedState.elements.length > 0) {
      const themeAdjustedElements = savedState.elements.map(el => ({
        ...el,
        color: el.themeColor ? getThemeColor(el.color, theme, el.themeColor) : el.color
      }));
  ...
  useEffect(() => {
    ...
    if ((theme === 'dark' && color === '#000000') || (theme === 'light' && color === '#FFFFFF')) {
      setColor(colors.defaultColor);
    }
      ...
      if (selectedElement) {
        const adjustedSelected = {
          ...
          color: selectedElement.themeColor ? 
            getThemeColor(selectedElement.color, theme, selectedElement.themeColor) : 
  ...
  const getElementAtPosition = useCallback((point: Point): DrawingElement | null => {
  ...
  const getResizeHandleAtPosition = useCallback((point: Point, element: DrawingElement): string | null => {
    if (element.type !== 'rectangle' && element.type !== 'circle') return null;
    ...
    if (element.type === 'rectangle') {
      ...
      // Top-left
      if (Math.abs(point.x - minX) < handleSize && Math.abs(point.y - minY) < handleSize) {
        return 'tl';
      }
      // Top-right
      if (Math.abs(point.x - maxX) < handleSize && Math.abs(point.y - minY) < handleSize) {
        return 'tr';
      }
      // Bottom-left
      if (Math.abs(point.x - minX) < handleSize && Math.abs(point.y - maxY) < handleSize) {
        return 'bl';
      }
      // Bottom-right
      if (Math.abs(point.x - maxX) < handleSize && Math.abs(point.y - maxY) < handleSize) {
        return 'br';
      }
  ...
  const isPointOnRotationHandle = useCallback((point: Point, element: DrawingElement): boolean => {
    if (element.type !== 'rectangle' && element.type !== 'circle') return false;
    ...
    if (element.type === 'rectangle') {
      ...
      return Math.abs(point.x - centerX) < handleSize && 
      ...
      return Math.abs(point.x - start.x) < handleSize && 
  ...
  const startDrawing = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
    ...
    if (tool === 'move') {
      ...
      if (element) {
        ...
        const handle = getResizeHandleAtPosition({ x: offsetX, y: offsetY }, element);
        if (handle) {
          setAction('resizing');
  ...
  const draw = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
    ...
    if (tool === 'move' && selectedElement) {
      ...
      if (action === 'moving' && startPoint) {
        ...
        if (selectedElement.type === 'rectangle') {
          ...
          switch (resizeHandle) {
            case 'tl': // Top-left
              ...
              break;
            case 'tr': // Top-right
              newPoints = [{ x: start.x, y: offsetY }, { x: offsetX, y: end.y }];
              break;
            case 'bl': // Bottom-left
              newPoints = [{ x: offsetX, y: start.y }, { x: end.x, y: offsetY }];
              break;
        ...
        else if (selectedElement.type === 'circle' && resizeHandle === 'edge') {
    ...
    // For drawing tools
      ...
      if (tool === 'brush' || tool === 'eraser') {
        ...
        if (tool === 'rectangle') {
          drawRectangle(ctxRef.current, updatedElement.points, updatedElement.color, updatedElement.size);
        } else if (tool === 'circle') {
          drawCircle(ctxRef.current, updatedElement.points, updatedElement.color, updatedElement.size);
        } else if (tool === 'line') {
          drawLine(ctxRef.current, updatedElement.points, updatedElement.color, updatedElement.size);
        }
  ...
  const drawSelectionHandles = (ctx: CanvasRenderingContext2D, element: DrawingElement) => {
    ...
    if (element.type === 'rectangle') {
      ...
      // Top-left
      ctx.beginPath();
      ctx.rect(minX - handleSize/2, minY - handleSize/2, handleSize, handleSize);
      ctx.fill();
      ctx.stroke();
      
      // Top-right
      ctx.beginPath();
      ctx.rect(maxX - handleSize/2, minY - handleSize/2, handleSize, handleSize);
      ctx.fill();
      ctx.stroke();
      
      // Bottom-left
      ctx.beginPath();
      ctx.rect(minX - handleSize/2, maxY - handleSize/2, handleSize, handleSize);
      ctx.fill();
      ctx.stroke();
      
      // Bottom-right
      ctx.beginPath();
      ctx.rect(maxX - handleSize/2, maxY - handleSize/2, handleSize, handleSize);
      ctx.fill();
      ctx.stroke();
  ...
  const stopDrawing = () => {
    ...
    if (tool === 'move' && selectedElement && action !== 'none') {
      ...
      const elementsWithoutSelected = elements.filter(el => el !== selectedElement);
